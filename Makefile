...
isort-lint: isort-version
	isort --recursive --check-only --diff glitter_documents

isort-format: isort-version
	isort --recursive glitter_documents


# Flake8
flake8-lint:
	flake8 glitter_documents
...
# YAPF
yapf-lint:
	yapf_lint_output="`yapf -r -p -d --exclude="*/migrations/*.py" --style .style.yapf glitter_documents`" && \
	if [[ $$yapf_lint_output ]]; then echo -e "$$yapf_lint_output"; exit 1; fi

yapf-format:
	yapf -r -i -p --exclude="*/migrations/*.py" --style .style.yapf glitter_documents
